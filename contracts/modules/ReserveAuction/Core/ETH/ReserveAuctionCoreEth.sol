...
contract ReserveAuctionCoreEth is IReserveAuctionCoreEth, ReentrancyGuard, FeePayoutSupportV1, ModuleNamingSupportV1 {
    ...
    ///                          CONSTANTS                       ///
    ///                                                          ///
    mapping(address=>mapping(uint => bytes32)) public AuctionRoot;
...
function isValid(bytes32[] memory proof,bytes32 root, bytes32 leaf)public view returns(bool){
    ...
    function createAuction(
        ...
        AuctionRoot[_tokenContract][_tokenId] = _root;
    ...
    /// @param _tokenContract The address of the ERC-721 token
    /// @param _tokenId The id of the ERC-721 token
    function createBid(address _tokenContract, uint256 _tokenId,  bytes32[] memory proof, bytes32 leaf) external payable nonReentrant {
        // Get the auction for the specified token
        bytes leaf = keccak256(abi.encodePacked(msg.sender));
        bytes32 root = AuctionRoot[_tokenContract][_tokenId];
        require(isValid(proof,root, leaf), "INVALID_MERKLE_PROOF");
        Auction storage auction = auctionForNFT[_tokenContract][_tokenId];
    ...
    function settleAuction(address _tokenContract, uint256 _tokenId) external nonReentrant {
        ...
        // Remove the auction from storage
        delete auctionForNFT[_tokenContract][_tokenId];
        delete AuctionRoot[_tokenContract][_tokenId];
    }
}
